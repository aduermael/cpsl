...
- [ ] 2b: Add `modeWorktrees` app mode. Wire `/worktrees` command in `handleCommand()`. On enter, fetch worktree list (async cmd calling `listWorktrees()`), populate the component. Esc returns to `modeChat`. Add `viewWorktrees()` method for the mode's View. Add `worktreeListMsg` message type.
