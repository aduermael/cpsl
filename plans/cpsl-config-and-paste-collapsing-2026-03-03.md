...
- [ ] 2b: Handle paste detection in `Update`. Listen for bubbletea's paste message type. When a paste arrives with char count ≥ config threshold, set a flag on the model (e.g., `pendingPaste bool`). When the user sends (Enter), check the flag to construct the message struct appropriately. Reset the flag after send.
