...
- [x] 5d: Restructure `buildSystemPrompt` to frame the main agent as an **orchestrator**. Add a section explaining: delegate complex subtasks to sub-agents, keep your own context lean, use the scratchpad as shared memory between agents, synthesize results. Sub-agents get a note to write key findings to the scratchpad. Add context window management guidelines.
