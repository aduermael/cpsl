...
- [ ] 1a: Create `models.go` with provider constants, `ModelDef` struct (provider, ID, display name), and the static model registry. Add a helper to filter models by available providers (given a set of configured API keys).
