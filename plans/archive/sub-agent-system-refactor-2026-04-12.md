...
- [x] 4c: Compact the result metadata header in `formatSubAgentResult`. Remove `[tokens: input=N output=M]` (tracked via EventUsage, not actionable by the main agent). Shorten to `[agent:<id> turns:<n/m> summary:<method>]`. Document the output file path pattern in `agent.md` instead of repeating it per result (~50 tokens saved per sub-agent call)
- [x] 4d: Add guidance to `prompts/tools/agent.md` for when to read full output: "If you need exact file paths, line numbers, code snippets, or error details not in the summary, read the full output at `.herm/agents/<agent_id>.md`"
- [x] 4e: Tests — verify outputs under 2KB are not summarized, verify structured summary format, verify compact result header format
